...
### 4. API 연동 및 비즈니스 로직 구현 시

- **구현 후 반드시 Playwright MCP로 테스트 수행** — 테스트 없이 다음 단계로 진행 금지
- 외부 API 연동(Notion API 등)은 실제 데이터 조회 시나리오를 Playwright로 검증
- 비즈니스 로직(금액 계산, 인증 플로우 등)은 정상/오류 케이스 모두 테스트
- 각 구현 단계 완료 후 중단하고 테스트 결과를 사용자에게 보고
- 테스트 실패 시 즉시 원인을 분석하고 수정 후 재테스트
...
### 3. Playwright MCP 활용 (테스트 단계 - 필수)

`mcp__playwright__*` 도구를 사용하여 API 연동 및 비즈니스 로직 구현 후 반드시 E2E 테스트를 수행합니다.

**활용 시점**:

- API 연동 함수 구현 완료 후 (Notion API, Route Handler 등)
- 인증 플로우 구현 완료 후 (로그인 → 쿠키 → 보호 라우트)
- 비즈니스 로직 구현 완료 후 (금액 계산, 데이터 파싱 등)
- 페이지 간 이동 및 리디렉션 구현 완료 후
- 오류 처리 로직 구현 완료 후
...
mcp__playwright__browser_fill_form({
  fields: [{ selector: 'input[type="password"]', value: 'test-password' }],
})
mcp__playwright__browser_click({ selector: 'button[type="submit"]' })
mcp__playwright__browser_wait_for({ text: '견적서 목록' }) // 리디렉션 후 텍스트 확인

// 3. 네트워크 요청 검증
mcp__playwright__browser_network_requests({}) // API 호출 확인

// 4. 오류 케이스 테스트
mcp__playwright__browser_navigate({ url: 'http://localhost:3000/invoice/invalid-id' })
mcp__playwright__browser_snapshot({}) // 오류 페이지 렌더링 확인
...
| 구현 항목 | 테스트 시나리오 |
|-----------|----------------|
| 로그인 플로우 | 올바른 패스워드 → 대시보드 리디렉션 확인 |
| 로그인 실패 | 틀린 패스워드 → 오류 메시지 표시 확인 |
| 인증 보호 라우트 | 비인증 상태 `/dashboard` 접근 → `/login` 리디렉션 확인 |
| Notion API 연동 | 견적서 목록 페이지에 실제 데이터 렌더링 확인 |
| 견적서 뷰어 | 유효한 ID로 접근 → 견적서 내용 렌더링 확인 |
| 잘못된 ID 처리 | 존재하지 않는 ID → 오류 페이지 확인 |
| URL 복사 | URL 복사 버튼 클릭 → 클립보드 복사 확인 |

### 4. Shadcn 활용 (UI 구성 단계 - 권장)
...
Phase 5: 코드 작성
   ↓
Phase 6: 테스트 (Playwright MCP) ← API 연동/비즈니스 로직 구현 시 필수
   ↓
Phase 7: 검토 및 최적화 (Sequential Thinking)
```

> ⚠️ **규칙**: API 연동 또는 비즈니스 로직을 포함하는 작업은 Phase 6 테스트를 반드시 수행해야 합니다. 테스트 통과 전까지 다음 Phase로 진행하지 않습니다.
...
**목표**: 구현된 API 연동 및 비즈니스 로직이 실제로 올바르게 동작하는지 검증

**단계**:

1. **개발 서버 확인**
   - `npm run dev` 실행 후 `http://localhost:3000` 접속 확인
   - `mcp__playwright__browser_navigate`로 초기 페이지 접근

2. **정상 케이스 테스트**
   - 주요 사용자 플로우를 순서대로 실행
   - 각 단계에서 `mcp__playwright__browser_snapshot` 으로 DOM 상태 확인
   - `mcp__playwright__browser_take_screenshot`으로 시각적 결과 캡처

3. **오류 케이스 테스트**
   - 잘못된 입력값, 존재하지 않는 ID, 인증 실패 등
   - 오류 페이지/메시지가 올바르게 표시되는지 확인

4. **네트워크 요청 검증**
   - `mcp__playwright__browser_network_requests`로 API 호출 확인
   - 응답 상태 코드 및 데이터 형식 검증

5. **테스트 결과 보고**
   - 통과/실패 항목 명시
   - 실패 시 원인 분석 및 수정 후 재테스트

**출력**: 테스트 통과 확인 및 결과 요약
...
### ✅ Playwright MCP 테스트 (API 연동/비즈니스 로직 포함 시 필수)
- [ ] 정상 사용자 플로우가 Playwright로 검증되었는가?
- [ ] 오류 케이스(잘못된 입력, 존재하지 않는 리소스)가 테스트되었는가?
- [ ] 인증 플로우(로그인 성공/실패, 보호 라우트 리디렉션)가 검증되었는가?
- [ ] API 네트워크 요청이 올바르게 발생하는지 확인되었는가?
- [ ] 테스트 결과(통과/실패 항목)가 사용자에게 보고되었는가?
...
### 7. Playwright MCP 테스트 결과 (API 연동/비즈니스 로직 포함 시 필수)
- 테스트한 시나리오 목록
- 각 시나리오 통과/실패 여부
- 실패 항목 원인 및 수정 내용
- 최종 테스트 통과 확인

### 8. 최종 검토 (Sequential Thinking)
- 구조 적절성 확인
- 성능 최적화 확인
- 확장 가능성 평가
- 개선 권장사항
